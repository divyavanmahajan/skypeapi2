...
import { loggerExtract } from '../../../libs/logging';

const debugJoin = arr => loggerExtract.debug(arr.join(' '));
 ...
 */
function extractXML(datasource) {
  const filePromise = new Promise(function(resolve, reject) {
    try {
      loggerExtract.info('Extracting XML from log');
      ...
      //   resolve(result);
      // });
      // loggerAPI.debug(JSON.stringify(datasource, null, 2));
      let macMediaLine = '';
      let macEndpoint = '';
